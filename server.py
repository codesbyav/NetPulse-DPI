...
This intentionally uses only the Python standard library so the UI can be run
without installing a web framework. The server serves /web and exposes a small
same-origin JSON API for inspection jobs.
"""
...
def engine_path() -> Path | None:
    ...
    if os.name == "nt":
        candidates.append(ROOT / "build" / "Release" / "dpi_engine.exe")
        candidates.append(ROOT / "build" / "dpi_engine.exe")
        candidates.append(ROOT / "dpi_engine.exe")
    else:
        candidates.append(ROOT / "build" / "dpi_engine")
        candidates.append(ROOT / "dpi_engine")
...
def parse_multipart(content_type: str, body: bytes) -> dict[str, tuple[str | None, bytes]]:
    """Parse the small browser FormData payload used by this app.

    Inputs are bounded by MAX_UPLOAD_BYTES and only one file is accepted.
    """
...
def run_engine(job_id: str, command: list[str], working_dir: Path, output_file: Path) -> None:
    ...
    try:
        ...
        log = "\n".join(output_lines)
        stats = {
            "total_packets": int(re.search(r"Total Packets:\s*(\d+)", log).group(1)) if re.search(r"Total Packets:\s*(\d+)", log) else 0,
            "total_bytes": int(re.search(r"Total Bytes:\s*(\d+)", log).group(1)) if re.search(r"Total Bytes:\s*(\d+)", log) else 0,
            "forwarded": int(re.search(r"Forwarded:\s*(\d+)", log).group(1)) if re.search(r"Forwarded:\s*(\d+)", log) else 0,
            "dropped": int(re.search(r"Dropped:\s*(\d+)", log).group(1)) if re.search(r"Dropped:\s*(\d+)", log) else 0,
        }
        with JOBS_LOCK:
            job = JOBS[job_id]
            job["log"] = log[-12000:]
    ...
    except Exception as error:  # Keep API errors in the job, rather than killing the server.
        with JOBS_LOCK:
...
class NetPulseHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if parsed.path == "/api/health":
            engine = engine_path()
            self.send_json({"engine_ready": bool(engine), "engine_path": str(engine) if engine else None})
            return
        ...
        if match:
            ...
            if not job_copy:
                ...
                return
            if match.group(2):
                output_file = Path(job_copy["output_file"])
                ...
                job_copy.pop("input_file", None)
                job_copy.pop("output_file", None)
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        engine = engine_path()
        if not engine:
            self.send_json({"error": "dpi_engine was not found. Build it first with CMake or set NETPULSE_ENGINE."}, HTTPStatus.SERVICE_UNAVAILABLE)
            return
        try:
        ...
        thread = threading.Thread(target=run_engine, args=(job_id, command, run_dir, output_file), daemon=True)
        thread.start()
